...
CREATE TRIGGER IF NOT EXISTS "JqlN_bd" BEFORE DELETE ON "JqlN" BEGIN
 SELECT RAISE(ABORT,'JqlN referenced') WHERE EXISTS(SELECT * FROM "JqlA" WHERE "n"=OLD."i");
END;
...
CREATE INDEX IF NOT EXISTS "JqlE_v_t" ON "JqlE"("v","t");
CREATE INDEX IF NOT EXISTS "JqlE_n" ON "JqlE"("n") WHERE "n" IS NOT NULL;
...
END;
CREATE TRIGGER IF NOT EXISTS "JqlEv_e_iu" INSTEAD OF UPDATE OF "e" ON "JqlEv" BEGIN
 UPDATE "JqlE" SET "e"=NEW."e" WHERE "i"=NEW."i" AND "e"!=NEW."e";
END;
CREATE TRIGGER IF NOT EXISTS "JqlEv_o_iu" INSTEAD OF UPDATE OF "o" ON "JqlEv" BEGIN
 UPDATE "JqlE" SET "o"=NEW."o" WHERE "i"=NEW."i" AND "o"!=NEW."o";
END;
CREATE TRIGGER IF NOT EXISTS "JqlEv_t_iu" INSTEAD OF UPDATE OF "t" ON "JqlEv" BEGIN
 INSERT INTO "JqlS"("v")SELECT NEW."v" WHERE NEW."t"=0 AND NOT EXISTS(SELECT * FROM "JqlS" WHERE "v"=NEW."v");
 UPDATE "JqlE" SET("t","v")=(NEW."t",IFNULL((SELECT "i" FROM "JqlS" WHERE(0,"v")=(NEW."t",NEW."v")),NEW."v"))WHERE "i"=NEW."i" AND "t"!=NEW."t";
END;
CREATE TRIGGER IF NOT EXISTS "JqlEv_v_iu" INSTEAD OF UPDATE OF "v" ON "JqlEv" BEGIN
 INSERT INTO "JqlS"("v")SELECT NEW."v" WHERE NEW."t"=0 AND NOT EXISTS(SELECT * FROM "JqlS" WHERE "v"=NEW."v");
 UPDATE "JqlE" SET "v"=IFNULL((SELECT "i" FROM "JqlS" WHERE(0,"v")=(NEW."t",NEW."v")),NEW."v")WHERE "i"=NEW."i" AND "v"!=IFNULL((SELECT "i" FROM "JqlS" WHERE(0,"v")=(NEW."t",NEW."v")),NEW."v");
END;
CREATE TRIGGER IF NOT EXISTS "JqlEv_n_iu" INSTEAD OF UPDATE OF "n" ON "JqlEv" BEGIN
 INSERT INTO "JqlN"("v")SELECT NEW."n" WHERE NEW."n" IS NOT NULL AND NOT EXISTS(SELECT * FROM "JqlN" WHERE "v"=NEW."v");
 UPDATE "JqlE" SET "n"=(SELECT "i" FROM "JqlN" WHERE "v"=NEW."v")WHERE "i"=NEW."i" AND "n" IS NOT(SELECT "i" FROM "JqlN" WHERE "v"=NEW."v");
END;
CREATE TRIGGER IF NOT EXISTS "JqlEv_ii" INSTEAD OF INSERT ON "JqlEv" BEGIN
 INSERT INTO "JqlS"("v")SELECT NEW."v" WHERE NEW."t"=0 AND NOT EXISTS(SELECT * FROM "JqlS" WHERE "v"=NEW."v");
 INSERT INTO "JqlN"("v")SELECT NEW."n" WHERE NEW."n" IS NOT NULL AND NOT EXISTS(SELECT * FROM "JqlN" WHERE "v"=NEW."v");
 INSERT INTO "JqlE"("i","e","o","t","v","n")VALUES(NEW."i",NEW."e",COALESCE(NEW."o",(SELECT MAX(o)+1 FROM "JqlE" WHERE e=NEW.e),0),NEW."t",IFNULL((SELECT "i" FROM "JqlS" WHERE(0,"v")=(NEW."t",NEW."v")),NEW."v"),(SELECT "i" FROM "JqlN" WHERE "v"=NEW."v"));
END;
